...
class modePool2(nn.Module):
    def __init__(self):
        super(modePool2, self).__init__()


    def forward(self, input):
        n_row = int(input.size()[2] / 2)
        n_col = int(input.size()[3] / 2)
        downsample = torch.zeros(n_row, n_col)
        for i in range(n_row):
            for j in range(n_col):
                neighbor4 = np.array(
                    [input[0, 0, 2 * i, 2 * j], input[0, 0, 2 * i, 2 * j + 1], input[0, 0, 2 * i + 1, 2 * j],
                     input[0, 0, 2 * i + 1, 2 * j + 1]])
...
class unetUp(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        if outputs2.size()[2] - inputs1.size()[2]:
            ...
            outputs1 = F.pad(outputs1, padding)
        if outputs2.size()[3] - inputs1.size()[3]:
